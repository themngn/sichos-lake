...
if [ "${SKIP_STEAM:-1}" = "1" ]; then
    ...
    # GameMode-by-default (see desktop-overrides/steam.desktop): every game
    # Steam launches auto-registers with the daemon via LD_PRELOAD, no
    # per-game "gamemoderun %command%" needed. Requires gamemode itself
    # (packages.txt) and group membership for its renice feature — see
    # gamemode.ini's own comment; group changes need a fresh login/reboot to
    # take effect, so just adding the group here isn't immediately live.
    install_file "$HERE/desktop-overrides/steam.desktop" \
        "$HOME/.local/share/applications/steam.desktop"
    # NEED_DESKTOP_DB_UPDATE's own consuming check already ran earlier
    # (Flatpak section above) — refresh directly here instead.
    if command -v update-desktop-database >/dev/null 2>&1; then
        update-desktop-database "$HOME/.local/share/applications"
    fi
    # Per-game shortcuts Steam itself generates ("Manage > Add desktop
    # shortcut") use a bare `Exec=steam steam://rungameid/<id>` — those
    # don't go through desktop-overrides/steam.desktop at all (different
    # .desktop file, one per game, owned/regenerated by Steam, not this
    # repo), so they'd launch gamemode-less if Steam wasn't already running
    # under our LD_PRELOAD. A ~/.local/bin/steam wrapper intercepts every
    # bare `steam` invocation instead — desktop-entry Exec resolution and an
    # interactive shell both search $PATH, and ~/.local/bin precedes
    # /usr/bin (confirmed via oh-my-posh above) — so this covers those
    # shortcuts, plus any terminal/script launch, on top of the desktop
    # override above.
    mkdir -p "$HOME/.local/bin"
    cat << 'EOF' > "$HOME/.local/bin/steam"
#!/usr/bin/env bash
exec env LD_PRELOAD=/usr/lib64/libgamemodeauto.so.0 /usr/bin/steam "$@"
EOF
    chmod +x "$HOME/.local/bin/steam"
    echo "    installed CLI wrapper ~/.local/bin/steam (GameMode LD_PRELOAD)"
    if ! id -nG "$USER" | grep -qw gamemode; then
        sudo usermod -aG gamemode "$USER"
        echo "    added $USER to the gamemode group (needs a fresh login/reboot to take effect)"
    fi
fi
