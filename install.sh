...
if [ "${SKIP_STEAM:-1}" = "1" ]; then
    ...
    # No LD_PRELOAD wrapper here anymore — see desktop-overrides/steam.desktop's
    # header comment for why the GameMode-by-default approach was reverted.
    # GameMode is opt-in per game via "gamemoderun %command%" in Launch
    # Options; the gamemode group membership below is still needed for that
    # (its renice feature — see gamemode.ini's own comment). Group changes
    # need a fresh login/reboot to take effect, so just adding the group
    # here isn't immediately live.
    if [ -e "$HOME/.local/bin/steam" ]; then
        rm -f "$HOME/.local/bin/steam"
        echo "    removed obsolete ~/.local/bin/steam GameMode wrapper"
    fi
